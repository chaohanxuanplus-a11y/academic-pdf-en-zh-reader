...
- 验证所有社区健康文件存在；开发态允许明确的 `PUBLIC_RELEASE_BLOCKED` 标记，但发布态发现联系人、私密安全渠道或替代联系人未配置时必须失败。
...
- 真实维护联系人、私密安全报告渠道和行为准则替代联系人必须在公开前由维护者填写。当前未知值使用机器可识别的 `PUBLIC_RELEASE_BLOCKED` 状态单独管理，不在正式文本中放入虚构姓名或邮箱；开发测试可继续，但发布测试必须失败。
...
- 真实维护联系人、安全渠道、行为准则替代联系人和所有法定署名未真实填写前，Release 工作流保持机器可验证的阻断状态；不得把开发态 `PUBLIC_RELEASE_BLOCKED` 标记带入发布包。
